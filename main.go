...
func main() {
	...
	directive := "insightscollector"
	objectPath := "/com/redhat/yggdrasil/Worker1/" + directive
	destinationName := "com.redhat.yggdrasil.Worker1." + directive

	_, err = prop.Export(conn, dbus.ObjectPath(objectPath), propertySpec)
	if err != nil {
	...
	if err := conn.Export(&worker, dbus.ObjectPath(objectPath), "com.redhat.yggdrasil.Worker1"); err != nil {
		log.Fatalf("cannot export com.redhat.yggdrasil.Worker1 interface: %v", err)
	}

	if err := conn.Export(introspect.Introspectable(ipc.InterfaceWorker), dbus.ObjectPath(objectPath), "org.freedesktop.DBus.Introspectable"); err != nil {
	...
	// com.redhat.yggdrasil.Worker1.insightscollector.
	reply, err := conn.RequestName(destinationName, dbus.NameFlagDoNotQueue)
	if err != nil {
